#!/usr/bin/env python3
"""
Fixed test script for SPX data fetching.
"""
...
def test_spx_data():
    """Test SPX data fetching for 5 days."""
    print("Testing SPX data fetching (5 days)...")
    
    # Create fetcher instance
    fetcher = DataFetcher("test_spx_data")
    print("✓ DataFetcher created successfully")
    
    # Test fetching stock data for SPX (5 days)
    print("\nFetching stock data for ^SPX (5 days)...")
    stock_data = fetcher.fetch_stock_data("^SPX", "5d", "1d")
    ...
    print("\nTesting data saving...")
    if not stock_data.empty:
        stock_file = fetcher.save_stock_data(stock_data, "^SPX", "5d", "1d")
        if stock_file:
...
if __name__ == "__main__":
    print("SPX Data Fetcher Test (Fixed)")
    print("=" * 50)
    ...
    if success:
        ...
        print("\nNext command to run:")
        print("poetry run python data_fetcher/src/data_fetcher.py ^SPX --period 2y --interval 1d")
    else:
